...
class DescribeCategories(object):
    ...
    @pytest.fixture(
        params=[
            ...
            ("c:barChart/c:ser/c:cat/c:lvl/(c:ptCount{val=2},c:pt{idx=1})", 1, 0),
        ]
    )
    ...
    def iter_fixture(self, Category_, category_):
        xChart = element("c:barChart/c:ser/c:cat/(c:ptCount{val=2},c:pt{idx=1})")
...
class DescribeCategory(object):
    ...
    @pytest.fixture(params=[(None, ""), ('c:pt/c:v"Foo"', "Foo")])
    ...
    @pytest.fixture(params=[(None, 42, 42), ('c:pt{idx=9}/c:v"Bar"', None, 9)])
    ...
    @pytest.fixture(params=[(None, ""), ('c:pt{idx=9}/c:v"Bar"', "Bar")])
